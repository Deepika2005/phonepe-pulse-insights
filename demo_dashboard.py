...
def load_geojson():
    ...
    for url in urls:
        try:
            ...
            continue
    raise ConnectionError("Could not load GeoJSON")
...
with st.sidebar:
    ...
    hr()
    st.markdown('<div style="color:#9b8ec4;font-size:12px;font-weight:600;margin-bottom:6px">Navigation</div>',unsafe_allow_html=True)
    st.markdown('<div style="color:#9b8ec4;font-size:11px;margin-bottom:4px">Select a page:</div>',unsafe_allow_html=True)
    page=st.selectbox("",["Home","Explore Data (Map)","Business Case Study"],label_visibility="collapsed")
    hr()
    st.markdown('<div style="color:#9b8ec4;font-size:10px;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px">Global Filters</div>',unsafe_allow_html=True)
    ...
    hr()
    st.caption("Source: PhonePe Pulse GitHub")

# ═══════════════════════════════════════════════════════
#  HOME
# ═══════════════════════════════════════════════════════
if page=="Home":
    st.markdown('<div class="pp-header"><div class="pp-logo">💜 PhonePe Pulse</div><div class="pp-sub">THE BEAT OF PROGRESS · TRANSACTION INSIGHTS DASHBOARD</div></div>',unsafe_allow_html=True)
    ...
    st.markdown(f'<div class="kpi-strip">{kpi("All Transactions",f"{tc/1e9:.2f}B","All years")}{kpi("Total Value",f"₹{ta/1e7:,.0f}Cr","Cumulative")}{kpi("Avg Value",f"₹{av:,.0f}","Per transaction")}{kpi("Registered Users",f"{ru/1e6:.1f}M","All years")}</div>',unsafe_allow_html=True)
    ...
    st.markdown(f'<div class="kpi-strip">{kpi("Transactions",f"{qtc/1e6:.2f}M",f"Q{sel_qtr} {sel_yr}")}{kpi("Amount",f"₹{qta/1e7:,.0f}Cr","Payment value")}{kpi("Registered Users",f"{qru/1e6:.2f}M",f"Q{sel_qtr} {sel_yr}")}{kpi("App Opens",f"{qao/1e9:.2f}B",f"Q{sel_qtr} {sel_yr}")}</div>',unsafe_allow_html=True)
    hr();c1,c2=st.columns([3,2])
    with c1:
        ...
        fig=go.Figure()
        fig.add_trace(go.Scatter(x=df_tr["Period"],y=df_tr["Ta"],mode="lines+markers",name="Amount",
            line=dict(color=P,width=2.5),fill="tozeroy",fillcolor="rgba(103,57,183,0.12)",
            marker=dict(size=6,color=C,line=dict(color=BG,width=1))))
        fig.update_layout(**PL,height=300,title="Transaction Amount Growth",title_font_color=TX,xaxis_tickangle=-45)
    ...
    with c4:
        sec("📱 Top 5 Brands")
        df_b5=q(f"SELECT Brands,SUM(Count) AS Users FROM aggregated_user WHERE Brands IS NOT NULL GROUP BY Brands ORDER BY Users DESC LIMIT 5")
...
#  EXPLORE DATA MAP
# ═══════════════════════════════════════════════════════
elif page=="Explore Data (Map)":

    # ── Top control row ─────────────────────────────────
    ctrl1,ctrl2,ctrl3,ctrl4=st.columns([1,1,1,3])
    with ctrl1: view=st.selectbox("",["Transactions","Users","Insurance"],label_visibility="collapsed",key="map_view")
    with ctrl2: tab_sel=st.selectbox("",["States","Districts","Postal Codes"],label_visibility="collapsed",key="map_tab")
    with ctrl3:
        # State filter — All India or a specific state
        all_states=["All India"]+sorted(q("SELECT DISTINCT State FROM aggregated_transaction ORDER BY State")["State"].str.title().tolist())
        sel_state=st.selectbox("",all_states,label_visibility="collapsed",key="map_state")

    # ── Load data based on view & state filter ───────────
    state_filter=""
    state_slug=""
    if sel_state!="All India":
        # Convert display title back to slug for SQL match
        state_slug=sel_state.lower().replace(" ","-").replace("&","-&-")
        # Find exact DB value
        db_states=q("SELECT DISTINCT State FROM aggregated_transaction")["State"].tolist()
        matched=[s for s in db_states if s.lower()==sel_state.lower() or STATE_MAP.get(s,"").lower()==sel_state.lower()]
        if matched:
            state_slug=matched[0]
            state_filter=f" AND State='{state_slug}'"

    if view=="Transactions":
        df_map=q(f"SELECT State,SUM(Transacion_count) AS Total_Count,SUM(Transacion_amount) AS Total_Amount FROM aggregated_transaction WHERE Year={sel_yr} AND Quater={sel_qtr}{state_filter} GROUP BY State")
        color_col="Total_Amount";count_lbl="Transactions";amt_lbl="Amount (₹)"
        df_cat=q(f"SELECT Transacion_type,SUM(Transacion_count) AS Cnt FROM aggregated_transaction WHERE Year={sel_yr} AND Quater={sel_qtr}{state_filter} GROUP BY Transacion_type ORDER BY Cnt DESC")
    elif view=="Users":
        df_map=q(f"SELECT State,SUM(Registered_user) AS Total_Count,SUM(App_opens) AS Total_Amount FROM aggregated_user WHERE Year={sel_yr} AND Quater={sel_qtr}{state_filter} GROUP BY State")
        color_col="Total_Count";count_lbl="Registered Users";amt_lbl="App Opens";df_cat=pd.DataFrame()
    else:
        if HAS_INS:
            df_map=q(f"SELECT State,SUM(Transacion_count) AS Total_Count,SUM(Transacion_amount) AS Total_Amount FROM aggregated_insurance WHERE Year={sel_yr} AND Quater={sel_qtr}{state_filter} GROUP BY State")
            df_cat=q(f"SELECT Transacion_type,SUM(Transacion_count) AS Cnt FROM aggregated_insurance WHERE Year={sel_yr} AND Quater={sel_qtr}{state_filter} GROUP BY Transacion_type ORDER BY Cnt DESC")
        else:
    ...
    # ── KPI strip ────────────────────────────────────────
    state_label=sel_state if sel_state!="All India" else "All India"
    st.markdown(f'<div class="kpi-strip">{kpi(f"{view} — {state_label}",f"{mc:,.0f}",count_lbl)}{kpi("Total Payment Value",f"₹{ma/1e7:,.0f}Cr","")}{kpi("Avg Transaction Value",f"₹{av:,.0f}","")}{kpi("Active States",f"{df_map[chr(83)+chr(116)+chr(97)+chr(116)+chr(101)].nunique()}","")}</div>',unsafe_allow_html=True)
    ...
    with map_col:
        try:
            ...
            fig=px.choropleth(df_map,geojson=geojson,featureidkey="properties.ST_NM",
                ...
                labels={"Total_Count":count_lbl,"Total_Amount":amt_lbl},height=480)
            ...
            st.info("🗺️ Showing fallback chart (no internet for map)")
            fb=df_map.nlargest(15,color_col).sort_values(color_col)
            fig=px.bar(fb,x=color_col,y="State_NM",orientation="h",height=450,
                       color=color_col,color_continuous_scale=["#2e1f6e","#00d4ff"],
                       title="State-wise Data")
            fig.update_layout(paper_bgcolor=BG,plot_bgcolor=CB,font_color=TX,
                margin=dict(l=8,r=8,t=36,b=8),
                coloraxis_colorbar=dict(title=amt_lbl,
                    tickfont=dict(color=TX,size=9),
                    title_font=dict(color=TX,size=9)))
        ...
        # ── State drill-down charts (shown below map when state selected) ──
        if sel_state!="All India" and state_slug:
            hr()
            sec(f"🔍 {sel_state} — District Drill-down")
            drill_tabs=st.tabs(["📍 Districts","📮 Pincodes","📈 Trend"])
            with drill_tabs[0]:
                if view=="Transactions":
                    df_drill=q(f"SELECT District,SUM(Count) AS Txns,SUM(Amount) AS Amt FROM map_transaction WHERE Year={sel_yr} AND Quater={sel_qtr} AND State='{state_slug}' GROUP BY District ORDER BY Amt DESC LIMIT 15")
                    if not df_drill.empty:
                        dc1,dc2=st.columns(2)
                        with dc1:
                            fig=px.bar(df_drill.sort_values("Amt"),x="Amt",y="District",orientation="h",
                                title=f"Districts by Amount — {sel_state}",height=400,
                                color="Amt",color_continuous_scale=["#2e1f6e","#6739B7","#00d4ff"])
                            fig.update_layout(**PL,title_font_color=TX,showlegend=False)
                            fig.update_xaxes(tickformat=",.0f")
                            st.plotly_chart(fig,use_container_width=True)
                        with dc2:
                            fig=px.bar(df_drill.sort_values("Txns"),x="Txns",y="District",orientation="h",
                                title=f"Districts by Count — {sel_state}",height=400,
                                color="Txns",color_continuous_scale=["#2e1f6e","#ff6b35","#ffd700"])
                            fig.update_layout(**PL,title_font_color=TX,showlegend=False)
                            fig.update_xaxes(tickformat=",.0f")
                            st.plotly_chart(fig,use_container_width=True)
                    else:
                        st.info(f"No district data for {sel_state} in Q{sel_qtr} {sel_yr}")
                elif view=="Users":
                    df_drill=q(f"SELECT District,SUM(Registered_user) AS Reg,SUM(App_opens) AS Opens FROM map_user WHERE Year={sel_yr} AND Quater={sel_qtr} AND State='{state_slug}' GROUP BY District ORDER BY Reg DESC LIMIT 15")
                    if not df_drill.empty:
                        dc1,dc2=st.columns(2)
                        with dc1:
                            fig=px.bar(df_drill.sort_values("Reg"),x="Reg",y="District",orientation="h",
                                title=f"Districts by Registered Users — {sel_state}",height=400,
                                color="Reg",color_continuous_scale=["#2e1f6e","#6739B7","#00d4ff"])
                            fig.update_layout(**PL,title_font_color=TX,showlegend=False)
                            fig.update_xaxes(tickformat=",.0f")
                            st.plotly_chart(fig,use_container_width=True)
                        with dc2:
                            fig=px.bar(df_drill.sort_values("Opens"),x="Opens",y="District",orientation="h",
                                title=f"Districts by App Opens — {sel_state}",height=400,
                                color="Opens",color_continuous_scale=["#2e1f6e","#ff6b35","#ffd700"])
                            fig.update_layout(**PL,title_font_color=TX,showlegend=False)
                            fig.update_xaxes(tickformat=",.0f")
                            st.plotly_chart(fig,use_container_width=True)
                    else:
                        st.info(f"No district data for {sel_state}")
                else:
                    if HAS_INS:
                        df_drill=q(f"SELECT District,SUM(Count) AS Policies,SUM(Amount) AS Amt FROM map_insurance WHERE Year={sel_yr} AND Quater={sel_qtr} AND State='{state_slug}' GROUP BY District ORDER BY Amt DESC LIMIT 15")
                        if not df_drill.empty:
                            fig=px.bar(df_drill.sort_values("Amt"),x="Amt",y="District",orientation="h",
                                title=f"Insurance Districts — {sel_state}",height=400,
                                color="Amt",color_continuous_scale=["#2e1f6e","#34d399","#f59e0b"])
                            fig.update_layout(**PL,title_font_color=TX,showlegend=False)
                            fig.update_xaxes(tickformat=",.0f")
                            st.plotly_chart(fig,use_container_width=True)
                        else:
                            st.info(f"No insurance district data for {sel_state}")

            with drill_tabs[1]:
                if view=="Transactions":
                    df_pin=q(f"SELECT EntityName AS Pincode,SUM(Count) AS Txns,SUM(Amount) AS Amt FROM top_transaction WHERE Year={sel_yr} AND Quater={sel_qtr} AND State='{state_slug}' AND EntityType='Pincode' GROUP BY EntityName ORDER BY Amt DESC LIMIT 15")
                elif view=="Users":
                    df_pin=q(f"SELECT EntityName AS Pincode,SUM(Registered_user) AS Txns,SUM(Registered_user) AS Amt FROM top_user WHERE Year={sel_yr} AND Quater={sel_qtr} AND State='{state_slug}' AND EntityType='Pincode' GROUP BY EntityName ORDER BY Amt DESC LIMIT 15")
                else:
                    df_pin=pd.DataFrame() if not HAS_INS else q(f"SELECT EntityName AS Pincode,SUM(Count) AS Txns,SUM(Amount) AS Amt FROM top_insurance WHERE Year={sel_yr} AND Quater={sel_qtr} AND State='{state_slug}' AND EntityType='Pincode' GROUP BY EntityName ORDER BY Amt DESC LIMIT 15")
                if not df_pin.empty:
                    fig=px.bar(df_pin.sort_values("Amt"),x="Amt",y="Pincode",orientation="h",
                        title=f"Top Pincodes — {sel_state}",height=420,
                        color="Amt",color_continuous_scale=["#2e1f6e","#6739B7","#ffd700"])
                    fig.update_layout(**PL,title_font_color=TX,showlegend=False)
                    fig.update_xaxes(tickformat=",.0f")
                    st.plotly_chart(fig,use_container_width=True)
                else:
                    st.info(f"No pincode data for {sel_state}")

            with drill_tabs[2]:
                if view=="Transactions":
                    df_trend=q(f"SELECT Year,Quater,SUM(Transacion_count) AS Cnt,SUM(Transacion_amount) AS Amt FROM aggregated_transaction WHERE State='{state_slug}' GROUP BY Year,Quater ORDER BY Year,Quater")
                    y_col="Amt";y_lbl="Amount (₹)"
                elif view=="Users":
                    df_trend=q(f"SELECT Year,Quater,SUM(Registered_user) AS Cnt,SUM(App_opens) AS Amt FROM aggregated_user WHERE State='{state_slug}' GROUP BY Year,Quater ORDER BY Year,Quater")
                    y_col="Cnt";y_lbl="Registered Users"
                else:
                    df_trend=q(f"SELECT Year,Quater,SUM(Transacion_count) AS Cnt,SUM(Transacion_amount) AS Amt FROM aggregated_insurance WHERE State='{state_slug}' GROUP BY Year,Quater ORDER BY Year,Quater") if HAS_INS else pd.DataFrame()
                    y_col="Amt";y_lbl="Amount (₹)"
                if not df_trend.empty:
                    df_trend["Period"]=df_trend["Year"].astype(str)+" Q"+df_trend["Quater"].astype(str)
                    fig=px.line(df_trend,x="Period",y=y_col,markers=True,
                        title=f"{view} Trend — {sel_state}",height=360,
                    ...
                    st.info(f"No trend data for {sel_state}")
    ...
    with right_col:
        ...
        if tab_sel=="States":
            ...
            dist_filter=f" AND State='{state_slug}'" if sel_state!="All India" and state_slug else ""
            df_dist=q(f"SELECT District,State,SUM(Amount) AS Amt FROM map_transaction WHERE Year={sel_yr} AND Quater={sel_qtr}{dist_filter} GROUP BY District,State ORDER BY Amt DESC LIMIT 10")
            ...
            sec("Top 10 Pincodes")
            pin_filter=f" AND State='{state_slug}'" if sel_state!="All India" and state_slug else ""
            df_pin=q(f"SELECT EntityName AS Pincode,State,SUM(Amount) AS Amt FROM top_transaction WHERE Year={sel_yr} AND Quater={sel_qtr} AND EntityType='Pincode'{pin_filter} GROUP BY EntityName,State ORDER BY Amt DESC LIMIT 10")
...
# ═══════════════════════════════════════════════════════
#  BUSINESS CASE STUDY  — single page, dropdown selector
# ═══════════════════════════════════════════════════════
elif page=="Business Case Study":
    st.markdown("<h2 style='color:#e8e0ff;font-weight:700;margin-bottom:4px'>Business Case Study</h2>",unsafe_allow_html=True)
    hr()
    ...
    st.markdown('<div style="color:#9b8ec4;font-size:13px;margin-bottom:6px">Choose Case Study</div>',unsafe_allow_html=True)
    case=st.selectbox("",CASES,label_visibility="collapsed")
    hr()

    # ─── CASE 1 ────────────────────────────────────────────────────────
    if case==CASES[0]:
        st.markdown('<div class="pp-header"><div class="pp-logo">📊 Case 1 — Decoding Transaction Dynamics on PhonePe</div><div class="pp-sub">Transaction behavior across states, quarters & payment categories</div></div>',unsafe_allow_html=True)
    ...
    # ─── CASE 2 ────────────────────────────────────────────────────────
        ...
        st.markdown('<div class="pp-header"><div class="pp-logo">📱 Case 2 — Device Dominance & User Engagement Analysis</div><div class="pp-sub">Brand preferences · engagement ratios · underutilised regions</div></div>',unsafe_allow_html=True)
        ...
        with t1:
            ...
            brand_qtrs=q("SELECT DISTINCT Quater FROM aggregated_user WHERE Brands IS NOT NULL ORDER BY Quater")["Quater"].tolist()
            if brand_years:
                bc1,bc2=st.columns([1,1])
                with bc1: sel_byr=st.selectbox("Brand Year",brand_years,index=len(brand_years)-1,key="brand_yr")
                with bc2: sel_bqtr=st.selectbox("Brand Quarter",brand_qtrs,index=0,key="brand_qtr")
            ...
            if not df_br.empty:
                ...
                st.warning("⚠️ No brand data found.")
        with t2:
            c1,c2=st.columns(2)
            with c1:
            ...
            with c2:
                ...
                st.plotly_chart(bh(df_lw.sort_values("Eng"),"Eng","State","⚠️ Low Engagement States",O),use_container_width=True)
    ...
    # ─── CASE 3 ────────────────────────────────────────────────────────
        ...
        st.markdown('<div class="pp-header"><div class="pp-logo">🛡️ Case 3 — Insurance Penetration and Growth Potential Analysis</div><div class="pp-sub">Growth trajectory · untapped markets · penetration analysis</div></div>',unsafe_allow_html=True)
        ...
        with t1:
            ...
            c1,c2=st.columns(2)
            with c1: st.plotly_chart(donut(df_is,"State","Amount","Amount — Top 10 States"),use_container_width=True)
            with c2: st.plotly_chart(bh(df_is.sort_values("Amount"),"Amount","State","Top 10 States by Amount",pal=True),use_container_width=True)
        with t2:
    ...
    # ─── CASE 5 ────────────────────────────────────────────────────────
        ...
        st.markdown('<div class="pp-header"><div class="pp-logo">👥 Case 5 — User Engagement and Growth Strategy</div><div class="pp-sub">Engagement scores · district drill-down · YoY growth</div></div>',unsafe_allow_html=True)
        ...
        with t1:
            ...
            with c2:
                ...
                tip("Summary",f"<b style='color:#34d399'>High</b>:{h} | <b style='color:#f59e0b'>Medium</b>:{m} | <b style='color:#ff6b35'>Low</b>:{l} states")
